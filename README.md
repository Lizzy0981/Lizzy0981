# Elizabeth Díaz Familia (Lizzy Familia) 👋

**Sustainable AI Scientist · AI Data Scientist · Sustainable Intelligence & BI**

Científica de datos y desarrolladora. Me especializo en medir el impacto ambiental de la inteligencia artificial — carbono, agua, energía, residuos electrónicos y minerales críticos. Trabajo con datos reales, fuentes oficiales y código abierto.

---

## 📊 Trabajo reciente

**[Calculadora de Huella de Carbono en Modelos de IA](https://lizzy0981.github.io/calculadora-huella-carbono-ia/)**
50 modelos · 5 dimensiones de impacto · 11 idiomas · Datos IEA, NASA, publicaciones peer-reviewed



---

## 🚀 Stack

`Python` `JavaScript` `TypeScript` `Java` `SQL` `R` `React` `Vue` `Vite`
`Angular` `Node.js` `FastAPI` `Spring Boot` `Jupiter Notebook` `Plotly` `XGBoost`
`TensorFlow` `Scikit-learn` `PyTorch` `D3.js` `Chart.js` `Keras` `Matplotlib`
`PostgreSQL` `MongoDB` `Redis` `Supabase` `NumPy` `Brain.js` `Google Colab` `Pandas`
 
---

## 📫 Contacto

- 📧 Email: [lizzyfamilia@gmail.com](mailto:lizzyfamilia@gmail.com)
- 🐦 Twitter: [@Lizzyfamilia](https://twitter.com/Lizzyfamilia)
- 👔 LinkedIn: [in/eli-familia](https://linkedin.com/in/eli-familia)
- 🐙 GitHub: [Lizzy0981](https://github.com/Lizzy0981)
- 🌐 Portfolio: [lizzy0981.github.io](https://lizzy0981.github.io)

---

![GitHub stats](https://github-readme-stats.vercel.app/api?username=Lizzy0981&show_icons=true&theme=radical&hide_border=true)
